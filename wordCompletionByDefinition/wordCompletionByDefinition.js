...
//const final_url =
("https://sheets.googleapis.com/v4/spreadsheets/1Ot6zIoe66dUYzOqZ4IHSKxp6I_yf9IMh4chSoESdGEk/values/WordCompletion!A4:E?key=AIzaSyB5dtBwq-qojLDt5tGE9fMj53bQI7J3QPo");
fetch(final_url)
  .then((res) => {
    return res.json();
  })
...
getNewQuestion = () => {
  ...
  createBoxesByWordlength();
  document.getElementById("box1").focus();
  createCheckButton();
};

function createBoxesByWordlength() {
  definitionDiv.innerHTML = `<h2 style="color:black">${definition}</h2>`;
  boxes.innerHTML = `<h3 style="color:blue">${firstHalf}</h3>`;
  for (let index = 0; index < lengthOfSecondHalf; index++) {
    boxes.innerHTML += `
          <input
            type="text"
            ...
            maxlength="1"
            size="1"
            style="font-size: 20; color: blue; text-align: center"
            oninput="moveToNext(this, 'box${index + 2}')"
            id="box${index + 1}"
          />
          `;
  }
...
function createCheckButton() {
  ...
  check.innerHTML = `
        <button class= "btn btn-outline-success" onclick="combineLetters()">Check</button>
        `;
}
